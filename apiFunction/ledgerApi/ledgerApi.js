...
export const getLedger = async (page,searchData,setLoading=()=>{}) => {
    const token = Cookies.get("token");
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/ledger/getAllLedgerEntries?page=${page}&pageSize=${PAGE_LIMIT}&search=${searchData}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`,
